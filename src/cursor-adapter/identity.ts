...
export function updateCwd(newCwd: string): void {
    if (!newCwd || newCwd.length === 0) return;
    if (process.env.ECORELAY_CWD && process.env.ECORELAY_CWD.length > 0) return;
    if (newCwd.includes("..")) return;
    const normalized = path.resolve(newCwd);
    try {
...
export function initialPeerName(cwd: string): string {
    ...
    if (cached && cached.startsWith("cursor-")) {
    ...
    return safeName(`cursor-${cleanBase}`) ?? "cursor-session";
}
