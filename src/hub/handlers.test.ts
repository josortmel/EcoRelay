...
describe("hub handlers", () => {
    ...
    test("broadcast fans out incoming_ask to other peers with broadcast_id", async () => {
        const a = await rawConnect(sockPath);
        const b = await rawConnect(sockPath);
        const c = await rawConnect(sockPath);
        a.send({
    ...
    test("sendTo to a destroyed socket does not crash hub", async () => {
        ...
        // Send a message to the dead peer; hub must not throw.
        a.send({ type: "send", to: "bob", text: "hi" });
        // Hub must respond with send_ack (delivered if close not yet processed, queued otherwise)
        const reply = await a.next();
        expect(reply.type).toBe("send_ack");
    ...
    test("broadcast shares broadcast_id as thread_id across per-recipient asks", async () => {
        const a = await rawConnect(sockPath);
        const b = await rawConnect(sockPath);
        const c = await rawConnect(sockPath);
        a.send({
